<?php
session_start();
  if (isset($_SESSION ['nom'])&&isset($_SESSION ['id'])&&isset($_SESSION['password'])&&isset($_SESSION ['photo'])&&isset($_SESSION ['bio'])) {
include("connexion.php");
$output='';

$nom=$_SESSION ['nom'];
    $id=$_SESSION ['id'];
   $requette="SELECT  lib_sujet FROM centreint WHERE id_utilisateur='$id';";
   $query2=mysqli_query($link,$requette);
   while ($row1=mysqli_fetch_array($query2))
   {
   	    $lib=$row1['lib_sujet'];
        $query=mysqli_query($link,"SELECT * FROM centreint WHERE lib_sujet='$lib' and id_utilisateur!='$id';" );
            while ($row=mysqli_fetch_array($query)) {
             $idd=$row['id_utilisateur'];
             $query3="SELECT * from follow where id_utilisateur='$id';";
              $r=mysqli_query($link,$query3);
            while ($ligne=mysqli_fetch_array($r)) {
             if ($ligne['following']!=$idd) {
             	 $query4=mysqli_query($link,"SELECT * FROM utilisateur WHERE id_utilisateur='$idd';" );
      $count= mysqli_num_rows($query4);
        if ($count==0) {
          $output= 'Il n\'y a pas de résultat';
        }else{
        	while ($ligne1=mysqli_fetch_array($query4)) {
                $nom=$ligne1['nom'];
            $nom1=str_replace(' ', '', $nom);;
                ...
                $output.="
        <a href='action11.php?id=$idd&do=follow'><input type=button class=bouton  value=Follow ></a>
        </div><br>";
             }}}
        }}}}
        ...
        $output1='';
   $requette3="SELECT  following FROM follow WHERE id_utilisateur='$id';";
   $query3=mysqli_query($link,$requette3);
   while ($row1=mysqli_fetch_array($query3))
   {
      ...
      $count1= mysqli_num_rows($query4);
        if ($count1==0) {
          $output1= 'Il n\'y a pas de résultat';
        }
        else{
          while ($row=mysqli_fetch_array($query4)) {
            ...
            $id1=$row['id_utilisateur'];
                ...
                $output1 .= '<div class=row><div class=col-8>'.$ph.'<b>'.$nom.'</b><i class="fa fa-check-circle-o"></i><br><span>@'.$nom1.'</span><br> '.$bio.'<br></div>';
                $output1.="<div class=col-3>
        <a href='message.php'><input type=button id=mesage class=contacter  value=contacter ></a>
        </div></div><br>";
        }}}
          mysqli_close($link);?>
<!DOCTYPE html>
	...
	<div class="container">
		<div class="options-menu">
			<div class="option-center">
				...
				<div class="option">
					...
					<div><a href="" title="Explorer"><i class="fa fa-hashtag">Explorer</i></a></div>
       ...
       echo $output;?>
						...
						<div class="image-video">
							<img src="photos/joebiden@president.com.png" alt="">
						</div>
